...
import { instagramCommentWebhookSchema } from "@/lib/validation/webhooks";
import { handleInstagramCommentWebhook } from "@/server/modules/webhooks/service";
...
export const POST = withApiErrorHandling(async (request: Request) => {
  ...
  const body = instagramCommentWebhookSchema.parse(JSON.parse(rawBody || "{}"));
  return jsonOk(await handleInstagramCommentWebhook(body));
});
